...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Doctor => {
            commands::doctor::run(json_output).await
        }
        Commands::Loopback { input, duration } => {
            commands::loopback::run(input, duration).await
        }
        Commands::Run { config, foreground } => {
            commands::run::run(config, foreground).await
        }
        Commands::Version => {
            commands::version::run(json_output)
        }
        Commands::Engines => {
            commands::engines::run(json_output).await
        }
        Commands::Status => {
            commands::status::run(json_output).await
        }
    }
}
