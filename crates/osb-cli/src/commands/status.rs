...
pub async fn run(json_output: bool) -> Result<()> {
    ...
    if json_output {
        ...
        let running_text = if status.daemon_running { "running" } else { "stopped" };
        println!("Daemon: {} {}", running_icon, running_text);
        println!();
        
        println!("Audio:");
        println!("  Capture:     {}", if status.audio.capture_active { "active" } else { "inactive" });
        println!("  Playback:    {}", if status.audio.playback_active { "active" } else { "inactive" });
        println!("  Virtual Mic: {}", if status.audio.virtual_mic_active { "active" } else { "inactive" });
        println!();
